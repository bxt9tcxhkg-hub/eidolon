...
| **Ist** | Signature-Object und Presence (`idle` / `thinking` / `acting` / `waiting` / `blocked` / `done`) aus Operate (`describeOperatePresence` → `setEidolonPresence`). Chat-Turn-Status `denkt` / `arbeitet` / `antwortet` über `GET /chat/turn-status`. `POST /chat` setzt `denkt` und `antwortet`. Die eine Live-Chat-Marke (`#chat-eidolon-presence`) sitzt links neben der Eidolon-Senderzeile im Transcript (letzte Assistant- oder Status-Zeile); ältere Eidolon-Zeilen nutzen dasselbe Still ohne eigenen Canvas/WebGL-Kontext. Die Sidebar-Signature bleibt sekundär. Composer-Status (`#chat-agent-status`) ist Text (`denkt…` / Phasen), keine große Presence-LED. Innere Bewegung kommt aus `eidolon-presence.js`: WebGL-Curl-/Domain-Warp der Still-Texel (Canvas-2D-Gitter-Warp als Fallback) plus ein Gold-Mote mit Idle-Drift und Phasen-Gaze. Die Textur lädt immer den PNG-Pfad (`img[src]`), nie das `<picture>`-WebP, damit Safari/iOS WebGL nicht auf einer WebP-Quelle scheitert; schlägt WebGL fehl, bleibt Canvas2D sichtbar lebendig. `data-presence-engine` meldet `webgl` / `canvas2d` / `still`. `/assets/eidolon-presence.js` ist per Query-Version und `Cache-Control: no-cache` cache-bust. Phasen kommen nur aus `data-turn-phase` (`setEidolonTurnPhase`). Idle: sichtbarer Tintenfluss und wanderndes Licht-Mote innerhalb 1–2s bei 36px Transcript / 42px Sidebar, Gaze nur wenn der Composer fokussiert ist. `denkt`: schnellere Turbulenz, Blick zum Composer. `antwortet`: ruhigerer Fluss, Blick auf den eigenen Turn-Text. `prefers-reduced-motion` und Settings `ui.animations=off` zeigen nur das genehmigte Still; unsichtbare Marks pausieren. Healing-/LLM-Probleme erscheinen in Connection/Systemstatus, nicht als Fake-Erfolg. |
| **Gap innerhalb der Phase** | `arbeitet` wird im normalen `/chat`-Pfad **nicht** gesetzt; nur der Self-Reflection-Chat setzt `PHASE_ARBEITET`. Die UI faked diese Phase nicht. Keine Emotions-, Stimmen- oder Tool-Work-Visuals. Kein CSS-Pan/Zoom des Still-Bitmaps als Haupteffekt. Presence bleibt eine Marke (36px Transcript, 42px Sidebar, max. 56px), kein Idle-Hero. |
...
- `already in Eidolon`: dunkle Schale, Chat-Tür statt Dashboard, genehmigtes Presence-Still als Marke neben dem Chat-Titel und in der Sidebar-Signature, innere Tintenbewegung (WebGL-Warp, 2D-Fallback; PNG-Textur, nicht Picture-WebP) plus interaktives Licht-Mote an `denkt` / `arbeitet` / `antwortet` gebunden, Idle-Swirl/Mote bei Markengröße lesbar, Chat-Status `denkt…` / `antwortet` am Turn, Action-Motion nur nach Mutation, `prefers-reduced-motion` + Settings `ui.animations` → statisches Still, `data-presence-engine` ehrlich.
...
- Der Chat-Avatar lebt links neben der Eidolon-Senderzeile im Transcript: die letzte Assistant-/Status-Zeile trägt die eine Live-Marke (`#chat-eidolon-presence`); ältere Eidolon-Zeilen zeigen dasselbe Still ohne eigenen WebGL-Kontext. Die Sidebar-Signature bleibt sekundär. Composer-Status bleibt Text (`denkt…`). Kein Hero und keine Status-LED neben dem Input.
