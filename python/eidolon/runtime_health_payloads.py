...
def health_payload(*, server_start: float, goal_stats: dict, backup_stats: dict, healing_state: dict, quic_status: dict, caps: list[dict], certs: dict, builtin_skills: list[dict], human_duration, http_port: int, quic_port: int) -> dict:
    ...
    problems: list[str] = []
    if not certs.get('complete'): problems.append('Zertifikate unvollständig')
    if certs.get('chain_valid') is False: problems.append('Zertifikatskette ungültig')
    if backup_stats.get('count', 0) < 1: problems.append('kein Backup vorhanden')
